...
def main():
    # Get inputs
    api = OpenMeteoAPI()
    backtest, location, forecast, cl, kde_factor, min_prob, models, bias, multiplier, history = streamlit_app.get_top_of_page(api.get_models())

    if backtest:
        try:
            back_df = api.pull_backtest(forecast,location,models,history)
            num_success, num_fail = back_test(back_df,min_prob,kde_factor,bias,multiplier)
